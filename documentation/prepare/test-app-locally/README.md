...
- You have prepared the SAP HANA Cloud properties for a JDBC connection as described in section [Get and Configure your HANA Cloud Instance](/documentation/prepare/configure-hana/README.md).
- You have an SAP S/4HANA Cloud system or a Business Partner mock server up and running as described in section [SAP S/4HANA or Business Partner Mock Server](../../prepare/configure-s4/README.md).
...
3. Maintain your SAP HANA Cloud JDBC connection properties in the `db.*` section. This should look like this:
   ...
   See [How to find JDBC Connection Properties](../../prepare/configure-hana#how-to-find-jdbc-connection-properties). 
...
1. Open a command line window and change to directory to [code/backend](../../../../code/backend) containing the main [pom.xml](../../../../code/backend/pom.xml). Run the following Maven command: 
   ...
   > Info: When running this command the first time, a lot of JAR files will be downloaded to your local Maven repository. 
...
To understand how this works, see section [Database Service](../../../explore/db-service/README.md))
...
2. The class [BPService.java](../../backend/bp-service/src/main/java/dev/kyma/samples/easyfranchise/bpservice/BPService.java) just queries for business partners. For a more detailed explanation, see the section [Business Partner Service](/documentation/explore/bp-service/README.md).
    ...
    As result you should get a JSON-based list of business partners. 
...
1. Let's create the mentor Peter Miller. The following curl should return `200 OK`. 
   ...
   Remember that the original REST call would have to provide the tenantid via header ```-H 'x-tenant-id: {tenantId}'```. In the local run it's taken form the `hiddenconfig.properties` file.
...
### Prerequisites 
- You have a node installed as described in the section [Development Setup](../../prepare/set-up-local-environment/README.md)
...
Add your username/password below and run the following command in a terminal:
...
The Email service should listen to http://localhost:3002. Checkout the [API Endpoints](/documentation/explore/email-service#list-of-api-endpoints) in the Explore section.
