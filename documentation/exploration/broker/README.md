...
- Two placeholders `<provider-subdomain>` and `<cluster-domain>` are defined below, and will be replaced by deployment script. Please refer to section [ManualDeployment](../../step-by-step-guide/run-app-in-kyma/deployment/ManualDeployment.md) for more details.
